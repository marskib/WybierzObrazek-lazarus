...
type
  ...
  TMojImage = Class(TImage)
    ...
    private
      Lapka : TLapka;      //Lapka/Podpowiedz wystawiana przez Obrazek (jesli jest to wlasciwy obrazek przebywajacy w OG)
...
Begin
  ...
  if FJestLapka then begin //Zaczynamy mrugac Lapka
    if (Lapka.Parent = Nil) then Lapka.Parent := Self.Parent; //zeby bylo widac, jesli nie ma jeszcze Parenta; {parentem Self-a jest FOperacje)
    Lapka.UstawKolorObramowania(Self.Parent.Color);
    Lapka.PolozNaXY_i_Wymiaruj(Left-5,Top-5, Width+10,Height+10);
    Lapka.SendToBack(); //bo inaczej Lapka przykrywa obrazek i nie da się przesuiwac obrazka ...
    Lapka.Mrugaj();
  end
...
Begin
  If ssLeft in Shift then Begin  //LewyKlawiszMyszy wscisniety
    ...
    if JestLapka then begin
      Lapka.Left:=Lapka.Left+dx;
      Lapka.Top :=Lapka.Top+dy;
    end;
    {}
    GetCursorPos(poz);
  end;
...
Begin
  ...
  WITH FOperacje do Begin //dla uproszczenia zapisow
    ...
    if Sprawdzacz.IlewObsz=1 then begin
      Ramka.JestLapka := FParametry.CBPodp.Checked;                  //Lapka 'ramkowa'
      Self.JestLapka  := FParametry.CBPodp.Checked;
...
(* Po zmianie tla na FParametry trzeba uczynic Lapke widzialną(!) *)
Begin
  Lapka.UstawKolorObramowania(BiezaceTlo);
End;
...
Begin
  ...
  Lapka := TLapka.WlasnyCreate(stRectangle);
...
Begin
  ...
  if lKrok<1 then begin   //zatrzymanie Timera
    ...
    With FOperacje do
      tabOb[idWylos].Lapka.WznowMruganie();
  end;
End;
